...
def test_single_study(project_name):
    ...
    try:
        ...
        end_time = time.time()
        print(f"  [OK] Data extraction completed (Time taken: {end_time - start_time:.2f} seconds)")
        ...
        end_time = time.time()
        print(f"  [OK] Data insertion completed (Time taken: {end_time - start_time:.2f} seconds)")
        
        # Step 6: Verify data insertion
        print("  -> Verifying data insertion...")
        verify_insertion()
        print("  [OK] Data insertion verified")
        
        print(f"[OK] Test for study '{project_name}' completed successfully")
    except Exception as e:
        print(f"[ERROR] Error testing study '{project_name}': {str(e)}")
...
def main():
    ...
    for study_name in studies:
        try:
            ...
            print(f"[OK] Study '{study_name}' processed successfully")
        except Exception as e:
            print(f"[ERROR] Error processing study '{study_name}': {str(e)}")
            continue
    ...
    for study_data in all_study_data:
        ...
        try:
            ...
            total_inserted += 1
            print(f"[OK] Study '{study_name}' data inserted successfully")
        except Exception as e:
            print(f"[ERROR] Error inserting data for study '{study_name}': {str(e)}")
            continue
